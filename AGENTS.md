...
Prefer `dataclass` for simple structured data. Initialize debug-level logging by default for
the app. Use modern Python type hints, but omit obvious `-> None` annotations and avoid exact
constant annotations when the value is self-explanatory. Add docstrings only when a function or
class name does not already make the intent clear.
...
Files generated by test, validation, or probe runs may be removed without confirmation.
